...
#BUG: eval_loss cannot be overfit to 50 training examples
#generated by a a network self-play. 0.2-0.3 training error remains.
#SOLVED: In TicTacToe, most data have the same input but different
...
class Training_Environment:
    ...
    def test_in(self):
        ...
        self.random_win_rates.append(
            compare(self.game,
                    [default_agent, RandomAgent()], 20,
                    normalize=True)[default_agent])
        
        self.evaluator_random_win_rates.append(
            compare(self.game,
                    [evaluator_agent, RandomAgent()], 20,
                    normalize=True)[evaluator_agent])
    ...
    def training(self):
        ...
        for i in range(1, 1 + self.loop_count):
            ...
            if hasattr(self, 'x_pool'):
                self.generate_games(self.new_states,
                                    ret=None, append_to_pool=True)
            else:
                self.generate_games(self.initial_pool,
                                    ret=None, append_to_pool=True)
