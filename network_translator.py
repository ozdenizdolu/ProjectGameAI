...
import game.reversi.translator as rev_tr
...
class StandardReversiTranslator:
    ...
    def dists_to_tensor(self, move_distributions, device):
        return torch.cat(
          [rev_tr.dist_to_tensor(distribution).unsqueeze(0)
                          for distribution in move_distributions]).to(device)
    
    def evals_to_tensor(self, states, evaluations, device):
        return torch.cat(
            [rev_tr.eval_to_tensor_wrt_white(evaluation).reshape(1,1)
             for evaluation in evaluations], dim=0).to(device)


    def tensor_to_dists(self, states, legal_moves_list, moves_tensor):
        """
        Returns a list of move distribution dictionaries from the given move
        ...
        ----------
        states : list
            A list of TicTacToeState's. Each state in this list is
            associated with the corresponding move distribution in the
            moves_tensor.
        legal_moves_list : list
            A list of list of legal moves for each state in states.
        moves_tensor: tensor
            A two dimensional tensor where each move distribution is indexed
            by the 0th dimension.
        ...
        if len(states) != len(legal_moves_list):
            raise ValueError(
                "len(states) should be equal to len(legal_moves_list")
        if len(states) != moves_tensor.shape[0]:
            raise ValueError(
                "len(states) should be equal to size of 0th dimension of"
                +" moves_tensor")
        return [rev_tr.tensor_to_dist(
                states[i], legal_moves_list[i], moves_tensor[i])
                for i in range(len(states))]
    ...
    def tensor_to_evals(self, states, evals_tensor):
        if evals_tensor.ndim != 2:
            raise ValueError("evals_tensor should have 2 dimensions")
        return [rev_tr.tensor_to_eval_wrt_white(evals_tensor[i])
                for i in range(evals_tensor.shape[0])]
